...
读读共享，读写互斥，写写互斥。
shared_mutex内部仅维护读线程和写线程的引用计数，不维护持有锁的线程，所以只要引用计数不为0，那任意一个线程都可以调用unlock_shared()和unlock()。
...
* `wait(_Lck)`: 阻塞当前线程。阻塞的时候会先释放锁然后再阻塞，被唤醒时会先自动获取锁。
* `wait(_Lck, _Pred)`: 如果_Pred不满足则阻塞当前线程，否则什么也不做。用法：`wait(lock, [](){return flag;});`。带谓词的wait已经自动处理了虚假唤醒，所以不需要在while里执行wait。
...
TODO：即使promise被销毁了，future也可以继续使用。
...
int main()
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        thread t([i, prom=move(p)]() mutable {  // lambda默认生成的是operator()() const{}
...
constexpr int TASK_MAXCNT = 100000;
class ThreadPool {
public:
    ThreadPool(int wcnt):m_wcnt(wcnt){
        for (int i = 0; i < wcnt; i++) {
            workers.emplace_back(bind(&ThreadPool::work, this));
        }
    }
    ~ThreadPool(){
        Stop();
...
private:
    void work() {
        while (!STOP_FLAG.load()) {
            unique_lock<mutex> l(m_mutex);
            cv.wait(l, [this]{return STOP_FLAG.load() || tasks.size()>0;});
...
int main()
{
    ...
    pool.AddTask(task3, a);
    // auto fut = pool.AddTask(task4);
    // cout<<fut.get()<<endl;
...
class BlockQueue{
public:
    BlockQueue(int capacity):m_capacity(capacity), m_size(0), m_notfull(), m_notempty(){}
    void push(int num) {
        unique_lock<mutex> l(m_mutex);
        m_notfull.wait(l, [this]{return m_size<m_capacity;});
        bq.push(num);
        m_size++;
    ...
    int pop() {
        unique_lock<mutex> l(m_mutex);
        m_notempty.wait(l, [this]{return m_size>0;});
        int val = bq.front();
        bq.pop();
        m_size--;
...
private: 
    ...
    int m_size;
    mutex m_mutex;
    condition_variable m_notfull, m_notempty;
};
```
...
int capacity = 3;
mutex m;
condition_variable cv_notfull, cv_notempty;
void Producer() {
    static int i = 0;
    while (true) {
        this_thread::sleep_for(chrono::milliseconds(100));
        unique_lock<mutex> l(m);
        cv_notfull.wait(l, []{return static_cast<int>(q.size())<capacity;});
        q.push(i++);
...
void Cumsumer() {
    while (true) {
        unique_lock<mutex> l(m);
        cv_notempty.wait(l, []{return static_cast<int>(q.size())>0;});
        cout<<q.front()<<" ";
        q.pop();
...
int main()
{
    ...
    for (int i = 0; i < producerSize; i++) {
        thread t(std::bind(&Producer));
        t.detach();
    }
    for (int i = 0; i < cumsumerSize; i++) {
        thread t(std::bind(&Cumsumer));
        t.detach();
    }
    mutex m;
    unique_lock<mutex> l(m);
...
int main()
{
    ...
    int cumsumerSize = 3;
    for (int i = 0; i < producerSize; i++) {
        thread t(std::bind(&Produce));
        t.detach();
    }
    for (int i = 0; i < cumsumerSize; i++) {
        thread t(std::bind(&Cumsum));
        t.detach();
    }
    mutex m;
    unique_lock<mutex> l(m);
