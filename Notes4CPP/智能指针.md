...
**使用场景**
* **对象需要把“自己”以 `shared_ptr` 形式传出去**：比如注册回调/事件订阅时，外部 API 要求回调里持有一个 `shared_ptr` 才能保证回调执行期间对象存活。
* **对象内部创建异步任务**：把任务投递到线程池/消息队列时，需要捕获 `shared_ptr` 延长生命周期，避免任务执行时对象已析构（典型写法：`auto self = shared_from_this();`）。
* **需要返回/缓存指向自己的 `shared_ptr`**：例如实现链式 API、把自己放进容器或管理器里（管理器用 `shared_ptr` 管理对象）。

**为什么需要它（原因）**
主要原因是希望这个对象的所有`shared_ptr`共享同一个控制块。用裸指针实现显然不能做到共享同一个控制块。所以`enable_shared_from_this`的用法是：
1. 该对象首先被shared_ptr管理
2. 然后再调用shared_from_this获取该对象的shared_ptr。

`enable_shared_from_this` 的作用是：让对象内部能够拿到“和外部同一个控制块”的 `shared_ptr`。它内部保存了一个 `weak_ptr` 指向该控制块，`shared_from_this()` 实际上是对这个 `weak_ptr` 做一次 `lock()`。
...
public:
    void startAsync() {
        ...
        // 伪代码：投递到异步执行环境
        // threadPool.post([self]{ self->doWork(); });
    }
...
`shared_ptr` 的循环引用本质是：对象 A 持有 B 的 `shared_ptr`，对象 B 也持有 A 的 `shared_ptr`，导致两边的引用计数都无法归零，从而析构永远不会触发。

解决思路是：**把“非拥有关系”的那条边改为 `weak_ptr`**（通常是“反向指针 / 父指针 / 回指 / 观察者关系”）。`weak_ptr` 不增加引用计数，只是观察控制块；需要使用对象时再 `lock()` 临时提升成 `shared_ptr`。

下面是把前面的 `Boy/Girl` 例子改成可释放的版本（让其中一端用 `weak_ptr` 保存回指）：
...
class Boy {
public:
    void setGirlFriend(const shared_ptr<Girl>& g) {
        girlFriend = g; // 拥有关系：Boy 拥有 Girl（示例）
    }
...
class Girl {
public:
    void setBoyFriend(const shared_ptr<Boy>& b) {
        boyFriend = b; // 非拥有关系：只是回指/观察
    }

    void printBoyFriendAlive() const {
        if (auto sp = boyFriend.lock()) {
            // sp 是临时 shared_ptr：在这个作用域内确保对象存活
            cout << "Boy is alive\n";
        } else {
            cout << "Boy already destroyed\n";
        }
    }
...
void noLeak() {
    auto spBoy  = make_shared<Boy>();
    auto spGirl = make_shared<Girl>();

    spBoy->setGirlFriend(spGirl);
    spGirl->setBoyFriend(spBoy);
    // 退出函数后：spBoy/spGirl 释放，各自引用计数归零，对象可正常析构
}
```

要点：
* `weak_ptr::lock()`：如果对象还活着，返回一个非空 `shared_ptr`；否则返回空 `shared_ptr`。
* `weak_ptr::expired()`：只做“是否过期”的检查（并不获取对象），通常更推荐直接 `lock()` 一步到位。
* 选择哪一边改成 `weak_ptr`：看语义谁“拥有”谁。**拥有的一边用 `shared_ptr`，回指/观察的一边用 `weak_ptr`**。
